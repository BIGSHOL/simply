...
Task tool parameters:
- subagent_type: "backend-specialist"
- description: "Phase 1, T1.1: DB 스키마 구현"
- prompt: |
    ## 태스크 정보
    - Phase: 1
    - 태스크 ID: T1.1
    ...
    ```bash
    git worktree add ../simly-phase1-db -b phase/1-db
    cd ../simly-phase1-db
    ```

    ## TDD 요구사항 (Phase 1+ 필수!)
    반드시 TDD 사이클을 따르세요:
    1. RED: 테스트 먼저 작성 (tests/models/test_models.py)
    2. GREEN: 테스트 통과하는 최소 구현
    3. REFACTOR: 테스트 유지하며 코드 정리

    테스트 명령어: `pytest tests/models/ -v`
    ...
    - 완료 보고 형식에 맞춰 보고
    - 사용자 승인 후에만 main 병합
    - 병합 후 worktree 정리: `git worktree remove ../simly-phase1-db`
```
...
| subagent_type | 역할 |
|---------------|------|
| `backend-specialist` | FastAPI, 비즈니스 로직, AI API 연동 |
...
| subagent_type | 역할 |
|---------------|------|
| `psychology-content-specialist` | 테스트 콘텐츠 설계, 문항/결과 유형, 심리학적 근거 |
| `result-analysis-specialist` | 결과 분석 로직, AI 프롬프트 설계, 점수 체계 |
| `ux-flow-specialist` | 테스트 UX 플로우, 와이어프레임, 인터랙션 설계 |
